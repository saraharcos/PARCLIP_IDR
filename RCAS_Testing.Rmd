...
df2_m6A %>%
  dplyr::select(seqnames, start, end, name, width, strand) %>%
  write_tsv("df2_m6A_unfilt.bed", col_names = FALSE)

tb_un <- importBed('df2_m6A_unfilt.bed')


df2_non %>%
  dplyr::select(seqnames, start, end, name, width, strand) %>%
  write_tsv("df2_no_m6A_unfilt.bed", col_names = FALSE)

tb_un_n <- importBed('df2_no_m6A_unfilt.bed')
...
cvgList <- calculateCoverageProfileList(queryRegions = tb_un, 
...
cvgList_n <- calculateCoverageProfileList(queryRegions = tb_un_n, 
                                        targetRegionsList = txdbFeatures, 
                                        sampleN = 10000)
...
#Gene regions
```{r}
summary <- summarizeQueryRegions(queryRegions = tb_un, 
...
df_gene <- df %>%
  ...
  mutate(percent = round((count / length(tb_un)), 3) * 100)
...
ggplot2::ggplot(df_gene, aes(x = reorder(feature, -percent), y = percent)) + 
  ...
  labs(x = 'transcript feature', y = paste0('percent overlap (n = ', length(tb_un), ')')) +
  theme_bw(base_size = 14) + 
  theme(axis.text.x = element_text(angle = 90))

summary_n <- summarizeQueryRegions(queryRegions = tb_un_n, 
                                 txdbFeatures = txdbFeatures)
...
df_n_gene <- df_n %>%
  filter(feature %in% c("fiveUTRs", "cds", "threeUTRs")) %>%
  mutate(percent = round((count / length(tb_un_n)), 3) * 100)
...
ggplot2::ggplot(df_n_gene, aes(x = reorder(feature, -percent), y = percent)) + 
  ...
  labs(x = 'transcript feature', y = paste0('percent overlap (n = ', length(tb_un_n), ')')) +
  theme_bw(base_size = 14) + 
  theme(axis.text.x = element_text(angle = 90))
...
ggplot2::ggplot(df_all_genes, aes(x = feature, y = percent)) + 
  geom_bar(stat = 'identity', aes(fill = feature)) + 
  scale_fill_brewer(palette = "Accent") +
...
ggplot2::ggplot(df_all_exonintron, aes(x = feature, y = percent)) + 
  geom_bar(stat = 'identity', aes(fill = feature)) + 
  scale_fill_brewer(palette = "Accent") +

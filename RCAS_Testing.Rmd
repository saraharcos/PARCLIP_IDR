...
df2 <- GRanges(seqnames = r3_bw3$Chromosome[df2_ov$subjectHits], IRanges(start = r3_bw3$Start[df2_ov$subjectHits], end = r3_bw3$End[df2_ov$subjectHits]), strand = r3_bw3$Strand[df2_ov$subjectHits]) %>%
  keepStandardChromosomes(pruning.mode="coarse")
...
#Get gene names using UCSC known genes
```{r}
gencode_hg19 <- makeTxDbFromGFF(file = "~/files/PARpipe/files/gencode.v19.chr_patch_hapl_scaff.annotation.gtf", organism = "Homo sapiens")

txbg <- transcriptsBy(gencode_hg19, by = "gene") %>% unlist()

# transcriptsByGene = transcriptsBy(TxDb.Hsapiens.UCSC.hg19.knownGene, by="gene")
# tx_unlist = unlist(transcriptsByGene)
# 
# df2_tx_ov <- findOverlaps(tx_unlist, df2)
# 
# df2_names <- GRanges(seqnames = seqnames(df2[subjectHits(df2_tx_ov)]), 
#                      IRanges(start = start(df2[subjectHits(df2_tx_ov)]), 
#                              end = end(df2[subjectHits(df2_tx_ov)])), 
#                      tx_id = tx_unlist[queryHits(df2_tx_ov)]$tx_id)
# 
# m6A_tx_ov <- findOverlaps(tx_unlist, m6A) 
# 
# m6A_names <- GRanges(seqnames = seqnames(m6A[subjectHits(m6A_tx_ov)]), 
#                      IRanges(start = start(m6A[subjectHits(m6A_tx_ov)]), 
#                              end = end(m6A[subjectHits(m6A_tx_ov)])), 
#                      tx_id = tx_unlist[queryHits(m6A_tx_ov)]$tx_id)
# 
# 
# df2_t <- df2_names %>%
#   as_tibble
...
```{r}
df2_peakanno <- annotatePeak(df2, TxDb = txbg) %>%
  as_tibble()

m6A_peakanno <- annotatePeak(m6A, TxDb = txbg) %>%
  as_tibble()

df2_m6A <- subsetByOverlaps(df2, m6A) %>%
  annotatePeak(TxDb = txbg) %>%
  as_tibble()
